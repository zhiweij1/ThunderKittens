...
#if (DPCT_COMPAT_RT_MAJOR_VERSION >= 11) && (DPCT_COMPATIBILITY_TEMP >= 750)
      float y;
    y = sycl::tanh(x);
      return y;
#else
...
template<kittens::ducks::sv::all SV> static inline void init_bias(rt_fl<16,SV::length> &acc, const SV &bias) {
    #pragma unroll
    ...
    struct consumer {
        static void setup(consumer_setup_args<layout> args) { // setup locals for before the first iteration
            ...
            group<NUM_CONSUMER_WARPS>::load(
                args.scratch.bias, args.globals.bias,
                {(int)sycl::ext::oneapi::this_work_item::get_nd_item<3>()
                     .get_group(1)});
...
#pragma unroll
            ...
            if(warpgroup::warpid() == 0)
                tma::store_async(
                    args.globals.acc, args.finish.acc[warpgroup::groupid()],
                    {(int)item_ct1.get_group(2) * NUM_CONSUMER_WARPGROUPS +
                         warpgroup::groupid(),
                     (int)item_ct1.get_group(1)});
...
void runbenchmark(size_t M, size_t N, size_t K) {
    ...
    sycl::ext::oneapi::bfloat16 *h_bias_bf16 =
    ...
    for (int i = 0; i < N; ++i)
        h_bias_bf16[i] = sycl::ext::intel::math::float2bfloat16(h_bias[i]);

    dpct::get_in_order_queue().memcpy(d_A, h_A_bf16, M * K * 2).wait();
    dpct::get_in_order_queue().memcpy(d_B, h_B_bf16, K * N * 2).wait();
    dpct::get_in_order_queue().memcpy(d_bias, h_bias_bf16, N * 2).wait();
    ...
    /*
    DPCT1010:501: SYCL uses exceptions to report errors and does not use the
    error codes. The cudaGetLastError function call was replaced with 0. You
    need to rewrite this code.
    */
    dpct::err0 cudaStatus = 0;
    /*
    DPCT1000:500: Error handling if-stmt was detected but could not be
    rewritten.
    */
    if (cudaStatus != 0) {
        /*
        DPCT1009:502: SYCL reports errors using exceptions and does not use
        error codes. Please replace the "get_error_string_dummy(...)" with a
        real error-handling function.
        */
        /*
        DPCT1001:499: The statement could not be removed.
        */

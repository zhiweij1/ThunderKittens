...
/*
DPCT1128:433: The type "fwd_globals<128>" is not device copyable for copy
...
field "l" and non trivially copyable field "o" breaking the device copyable
...
template <int D>
struct sycl::is_device_copyable<fwd_globals<D>> : std::true_type {};

template <int D, bool is_causal>
/*
DPCT1110:428: The total declared local variable size in device function
fwd_attend_ker exceeds 128 bytes and may cause high register pressure. Consult
with your hardware vendor to find the total register size available and adjust
the code, or use smaller sub-group size to avoid high register pressure.
*/
SYCL_EXTERNAL void fwd_attend_ker(const fwd_globals<D> g, uint8_t *dpct_local) {
    ...
    int kv_head_idx = item_ct1.get_group(1) / g.hr;
    int seq_idx = item_ct1.get_group(2) * CONSUMER_WARPGROUPS;

    auto &qsmem_semaphore =
        *sycl::ext::oneapi::group_local_memory_for_overwrite<
            kittens::semaphore>(
            sycl::ext::oneapi::this_work_item::get_work_group<3>());
    auto &k_smem_arrived = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore[K::stages]>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    auto &v_smem_arrived = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore[K::stages]>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    auto &compute_done = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore[K::stages]>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    if (item_ct1.get_local_id(2) == 0) {
        ...
        for (int wg = 0; wg < CONSUMER_WARPGROUPS; wg++) {
            coord<q_tile> q_tile_idx = {static_cast<int>(item_ct1.get_group(0)),
                                        static_cast<int>(item_ct1.get_group(1)), (seq_idx) + wg,
                                        0};
            tma::load_async(q_smem[wg], g.q, q_tile_idx, qsmem_semaphore);
        }

        for (int j = 0; j < K::stages - 1; j++) {
            coord<k_tile> kv_tile_idx = {static_cast<int>(item_ct1.get_group(0)), kv_head_idx, j,
                                         0};
    ...
    if(warpgroupid == NUM_WARPGROUPS-1) {
        ...
        if(warpid == NUM_WORKERS-4) {
            for (auto kv_idx = pipe_idx - 1; kv_idx <= kv_iters; kv_idx++) {
                coord<k_tile> kv_tile_idx = {static_cast<int>(item_ct1.get_group(0)), kv_head_idx,
                                             kv_idx + 1, 0};
        ...
        if (warpid % 4 == 0) {
            coord<o_tile> o_tile_idx = {static_cast<int>(item_ct1.get_group(0)),
                                        static_cast<int>(item_ct1.get_group(1)),
        ...
        if (warpid % 4 == 0) {
            coord<l_col_vec> tile_idx = {static_cast<int>(item_ct1.get_group(0)),
                                         static_cast<int>(item_ct1.get_group(1)), 0,
                                         (seq_idx) + warpgroupid};
...
template <int D>
/*
DPCT1128:440: The type "bwd_prep_globals<128>" is not device copyable for copy
constructor, move constructor, non trivially copyable field "og", non trivially
copyable field "o" and non trivially copyable field "d" breaking the device
...
template <int D>
struct sycl::is_device_copyable<bwd_prep_globals<D>> : std::true_type {};

template <int D>
/*
DPCT1110:429: The total declared local variable size in device function
bwd_attend_prep_ker exceeds 128 bytes and may cause high register pressure.
Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
SYCL_EXTERNAL void bwd_attend_prep_ker(const bwd_prep_globals<D> g,
                                       uint8_t *dpct_local) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    auto __shm = (int *)dpct_local;
    tma_swizzle_allocator al((int*)&__shm[0]);
    ...
    auto &smem_semaphore = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    ...
    */
    ...
    if (warpid == 0) {
        for (int w = 0; w < 4; w++) {
            coord<o_tile> tile_idx = {static_cast<int>(item_ct1.get_group(0)),
                                      static_cast<int>(item_ct1.get_group(1)),
                                      static_cast<int>((item_ct1.get_group(2) * 4) + w), 0};
    ...
    */
    ...
    if (warpid == 0) {
        for (int w = 0; w < 4; w++) {
            coord<d_tile> tile_idx = {static_cast<int>(item_ct1.get_group(0)),
                                      static_cast<int>(item_ct1.get_group(1)), 0,
                                      static_cast<int>((item_ct1.get_group(2) * 4) + w)};
...
template <int D>
/*
DPCT1128:447: The type "bwd_globals<128>" is not device copyable for copy
constructor, move constructor, non trivially copyable field "q", non trivially
copyable field "k", non trivially copyable field "v", non trivially copyable
field "og", non trivially copyable field "qg", non trivially copyable field
"kg", non trivially copyable field "vg", non trivially copyable field "l" and
non trivially copyable field "d" breaking the device copyable requirement. It is
...
template <int D>
struct sycl::is_device_copyable<bwd_globals<D>> : std::true_type {};
...
stream_tile(auto &reg_tile, auto &smem_vec, int tic) {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        reg_tile.tiles[0][i].data[0] = *(sycl::float2*)&smem_vec[tic][base_col + 0];
        reg_tile.tiles[0][i].data[1] = *(sycl::float2*)&smem_vec[tic][base_col + 0];
        reg_tile.tiles[0][i].data[2] = *(sycl::float2*)&smem_vec[tic][base_col + 8];
        reg_tile.tiles[0][i].data[3] = *(sycl::float2*)&smem_vec[tic][base_col + 8];
    }
}
...
stream_sub_tile(auto &reg_tile, auto &smem_vec, int tic) {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        reg_tile.tiles[0][i].data[0] = base_ops::sub::template op<sycl::float2>(reg_tile.tiles[0][i].data[0], *(sycl::float2*)&smem_vec[tic][base_col + 0]);
        reg_tile.tiles[0][i].data[1] = base_ops::sub::template op<sycl::float2>(reg_tile.tiles[0][i].data[1], *(sycl::float2*)&smem_vec[tic][base_col + 0]);
        reg_tile.tiles[0][i].data[2] = base_ops::sub::template op<sycl::float2>(reg_tile.tiles[0][i].data[2], *(sycl::float2*)&smem_vec[tic][base_col + 8]);
        reg_tile.tiles[0][i].data[3] = base_ops::sub::template op<sycl::float2>(reg_tile.tiles[0][i].data[3], *(sycl::float2*)&smem_vec[tic][base_col + 8]);
    }
}
...
causal_mask(auto &reg_tile, int qo_idx) {
    ...
    int k_blk =
        (sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_group(2) *
         BWD_CONSUMER_WARPGROUPS * (tile_h / kittens::TILE_ROW_DIM<bf16>)) +
        ((kittens::warpid() / kittens::WARPGROUP_WARPS) *
         (tile_h / kittens::TILE_ROW_DIM<bf16>)) +
        (kittens::warpid() % kittens::WARPGROUP_WARPS);
         ...
         auto &dst, auto &bar, int kv_head_idx, int toc) 
{
    ...
    if (kittens::warpid() % 4 == 0) {
        coord<kg_tile> tile_idx = {
            static_cast<int>(item_ct1.get_group(0)), kv_head_idx,
            static_cast<int>((item_ct1.get_group(2) * BWD_CONSUMER_WARPGROUPS) +
                (kittens::warpid() / kittens::WARPGROUP_WARPS)),
            0};
...
template <int D, bool is_causal>
/*
DPCT1110:430: The total declared local variable size in device function
bwd_attend_ker exceeds 128 bytes and may cause high register pressure. Consult
with your hardware vendor to find the total register size available and adjust
the code, or use smaller sub-group size to avoid high register pressure.
*/
SYCL_EXTERNAL void bwd_attend_ker(const bwd_globals<D> g, uint8_t *dpct_local) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    auto __shm = (int *)dpct_local;
    tma_swizzle_allocator al((int*)&__shm[0]);
    ...
    const int kv_head_idx = (item_ct1.get_group(1)) / hr;

    auto &kv_b = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    auto &q_b = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore[2]>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    auto &o_b = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore[2]>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    auto &vec_b = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore[2]>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    auto &compute_done = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore[2]>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());
    auto &qg_ready = *sycl::ext::oneapi::group_local_memory_for_overwrite<
        kittens::semaphore>(
        sycl::ext::oneapi::this_work_item::get_work_group<3>());

    int tic = 0, toc = 1;
    const int q_start = (is_causal) ? (item_ct1.get_group(2) * 2) : (0);
    ...
    if (item_ct1.get_local_id(2) == 0) {
        ...
        for (int w = 0; w < BWD_CONSUMER_WARPGROUPS; w++) {
            coord<k_tile> tile_idx = {
                static_cast<int>(item_ct1.get_group(0)), kv_head_idx,
                static_cast<int>((item_ct1.get_group(2) * BWD_CONSUMER_WARPGROUPS) + w), 0};
            tma::load_async(k_smem[w], g.k, tile_idx, kv_b);
            tma::load_async(v_smem[w], g.v, tile_idx, kv_b);
        }

        coord<q_tile> tile_idx = {static_cast<int>(item_ct1.get_group(0)), static_cast<int>(item_ct1.get_group(1)),
                                  q_start, 0};
        ...
        coord<l_tile> vec_idx = {static_cast<int>(item_ct1.get_group(0)), static_cast<int>(item_ct1.get_group(1)),
                                 0, q_start};
        ...
        if (warpid % kittens::WARPGROUP_WARPS == 0) {
            for (auto qo_idx = q_start; qo_idx < qo_blocks; qo_idx++, tic ^= 1, toc ^= 1) {
                if (qo_idx + 1 < qo_blocks) {
                    coord<q_tile> tile_idx = {static_cast<int>(item_ct1.get_group(0)),
                                              static_cast<int>(item_ct1.get_group(1)), qo_idx + 1,
                                              0};
                    ...
                    coord<l_tile> vec_idx = {static_cast<int>(item_ct1.get_group(0)),
                                             static_cast<int>(item_ct1.get_group(1)), 0,
                                             qo_idx + 1};
            ...
            for (auto qo_idx = q_start; qo_idx < qo_blocks; qo_idx++, tic ^= 1, toc ^= 1) {
                ...
                coord<qg_tile> tile_idx = {static_cast<int>(item_ct1.get_group(0)),
                                           static_cast<int>(item_ct1.get_group(1)), qo_idx, 0};

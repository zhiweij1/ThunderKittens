...
struct test_mma_AB {
    ...
    template<int H, int W, int NW, gl_t GTL_A, gl_t GTL_B, gl_t GTL_C, typename _K> static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
...
struct mma_wrapper_2d {
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS, _K, args...>::value) {
            ...
            // run kernel
            /*
            DPCT1026:66: The call to cudaFuncSetAttribute was removed because

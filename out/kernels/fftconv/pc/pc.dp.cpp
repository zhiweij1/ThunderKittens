...
template<int _wg> struct fftconv_1024_layout { // 4096
    ...
    /*
    DPCT1128:329: The type "typename fft_1024_template::layout::globals" is not
    device copyable for copy constructor, move constructor, non trivially
    copyable field "o", non trivially copyable field "x", non trivially copyable
    field "kf", non trivially copyable field "f", non trivially copyable field
    "finv", non trivially copyable field "tw" and non trivially copyable field
    "twinv_t" breaking the device copyable requirement. It is used in the SYCL
...
struct fft_1024_template {
    ...
    // mine
    static inline void load_head_data(typename layout::scratch_block &scratch, const layout::globals &g, int head) {
    ...
    // tk
    SYCL_EXTERNAL static inline void
    common_setup(common_setup_args<layout> args) {
        int heads_handled =
            (args.globals.x.depth() + 131 -
             sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_group(2)) /
            132; // I am guaranteeing batch is handled by just one block.
    ...
    struct producer {
        ...
        SYCL_EXTERNAL static inline void load(producer_load_args<layout> args) {
            int iters_per_head = (args.globals.x.batch() + (NUM_CONSUMER_WARPGROUPS*4)-1) / (NUM_CONSUMER_WARPGROUPS*4);
            int head =
                (args.iter / iters_per_head) * 132 +
            ...
            if(warpgroup::warpid() == args.iter%4) {
                ...
                sycl::group_barrier(
                    sycl::ext::oneapi::this_work_item::
                        get_sub_group()); // memory must arrive before arrival
    ...
    struct consumer {
        ...
        /*
        DPCT1110:317: The total declared local variable size in device function
        compute exceeds 128 bytes and may cause high register pressure. Consult
        ...
        */
        ...
        compute(consumer_compute_args<layout> args) {
            ...
            int next_head =
                ((args.iter + 1) / iters_per_head) * 132 +
                sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_group(
                    2);
...
template<int _wg> struct fftconv_4096_layout { // 4096
    ...
    using fft_layout    = cgl<gl<bf16,  1,  1, 64, 64>>;
    /*
    DPCT1128:327: The type "typename fft_4096_template::layout::globals" is not
    device copyable for copy constructor, move constructor, non trivially
    copyable field "o", non trivially copyable field "x", non trivially copyable
    field "kf", non trivially copyable field "f", non trivially copyable field
    "finv", non trivially copyable field "tw" and non trivially copyable field
...
struct fft_4096_template {
    ...
    // tk
    SYCL_EXTERNAL static void common_setup(common_setup_args<layout> args) {
        int heads_handled =
            (args.globals.x.depth() + 131 -
             sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_group(2)) /
            132; // I am guaranteeing batch is handled by just one block.
    ...
    struct producer {
        SYCL_EXTERNAL static void setup(producer_setup_args<layout> args) {
        ...
        SYCL_EXTERNAL static void load(producer_load_args<layout> args) {
            ...
            if(warpgroup::warpid() == args.iter%4) {
                tma::expect_bytes(
                    args.inputs_arrived,
                    sizeof(args.input.x[0]) *
                        sycl::min((int)NUM_CONSUMER_WARPGROUPS,
    ...
    struct consumer {
        ...
        /*
        DPCT1110:318: The total declared local variable size in device function
        compute exceeds 128 bytes and may cause high register pressure. Consult
        with your hardware vendor to find the total register size available and
        adjust the code, or use smaller sub-group size to avoid high register
        pressure.
        */
        SYCL_EXTERNAL static void compute(consumer_compute_args<layout> args) {
        ...
        SYCL_EXTERNAL static void finish(consumer_finish_args<layout> args) {
            if (laneid() == 0) arrive(args.finish_finished);
        }
    };
};
...
void launch(typename fft_template<SEQ>::layout::globals G) {
    ...
    /*
    DPCT1026:487: The call to cudaFuncSetAttribute was removed because SYCL
    currently does not support corresponding setting.
    */
    dpct::dim3 grid(132);
    dpct::dim3 block(prototype::detail::NUM_THREADS_v<fftst>);
#ifdef TK_COMPILE_FFTCONV
    auto stream = at::cuda::getCurrentCUDAStream().stream();
    prototype::lcsf::kernel<fftst><<<grid, block, mem_size, stream>>>(G);
#else
    /*
    DPCT1049:319: The work-group size passed to the SYCL kernel may exceed the
    limit. To get the device limit, query info::device::max_work_group_size.
    Adjust the work-group size if needed.
    */
    /*
    DPCT1129:328: The type "typename fft_4096_template::layout::globals" is used
    in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please
    review the code.
    */
    /*
    DPCT1129:330: The type "typename fft_1024_template::layout::globals" is used
    in the SYCL kernel, but it is not device copyable. The
    sycl::is_device_copyable specialization has been added for this type. Please
    review the code.
    */

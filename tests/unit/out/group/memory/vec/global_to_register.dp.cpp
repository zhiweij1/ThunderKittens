...
struct vec_load_store {
    ...
    template<int S, int NW, gl_t GL, kittens::ducks::rt_layout::all L> static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        o_ref = i_ref; // overwrite the whole thing
    }
    template<int S, int NW, gl_t GL, kittens::ducks::rt_layout::all L> static void device_func(const GL &input, const GL &output) {

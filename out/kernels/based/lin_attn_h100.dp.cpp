...
/*
DPCT1128:402: The type "based_globals" is not device copyable for copy
...
field "o", non trivially copyable field "kv_a0", non trivially copyable field
"kv_a1" and non trivially copyable field "kv_a2" breaking the device copyable
...
void accumulate_a0(sv_fl<64> &a0_float, const ST &v) {
    ...
    for(int row = 0; row < ST::rows/4; row++) {
        ...
        sycl::float2 tmpf =
            sycl::float2(sycl::ext::intel::math::bfloat162float(tmp.x()),
                         sycl::ext::intel::math::bfloat162float(tmp.y()));
        acc.x() += tmpf.x();
        acc.y() += tmpf.y();
    }
    // __syncthreads();
...
static void mul_slice_row(rt_bf<1*16,4*16> &dst, const rt_bf<1*16,1*16> &src, const int starting_col) {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        for(int row_offset = 0; row_offset < 2; row_offset++) {
            ...
            /*
            DPCT1108:394: '__shfl_sync' was migrated with the experimental
...
static void mul_slice_col(rt_bf<16,64> &dst, const rt_bf<64,16,col_l> &src, const int target_row) {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        for(int col_offset = 0; col_offset < 2; col_offset++) {
            ...
            bf16_2 src_val = dst.tiles[0][i].data[2*col_offset + row_offset];
            /*
            DPCT1108:395: '__shfl_sync' was migrated with the experimental
            feature masked sub_group function which may not be supported by all
            compilers or runtimes. You may need to adjust the code.
            */
...
void based_linear_attention(const based_globals g,
    ...
    // initial load
    auto &bar = *sycl::ext::oneapi::group_local_memory_for_overwrite<semaphore>(
    ...
    if (warpid == 0) {
        ...
        int tile_idx = item_ct1.get_group(2) * n_blocks;
    ...
    for (int block = 0; block < n_blocks; block++, tic^=1, toc^=1) {
        ...
        for(int i = 0; i < 4; i++) {
            ...
            for(int j = 0; j < 2; j++) {
                ...
                sycl::float2 data;
                move<sycl::float2>::lds(data,
                                        a0_float_handle + col * sizeof(float));
        ...
        mul(q_src, q_src, sycl::ext::intel::math::float2bfloat16(0.25));
        ...
        mul(q_src, q_src,
            sycl::ext::intel::math::float2bfloat16(
                0.70710678118)); // divide by 2 for A2 here; the mul_slices

...
/*
DPCT1128:388: The type "based_globals" is not device copyable for copy
constructor, move constructor, non trivially copyable field "q", non trivially
copyable field "k", non trivially copyable field "v" and non trivially copyable
...
template <> struct sycl::is_device_copyable<based_globals> : std::true_type {};

template<kittens::ducks::st::all ST, int N_TILES>
void accumulate_a0(ST (&o)[N_TILES], sv_fl<ST::cols> &running_sum, const ST (&v)[N_TILES]) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    float acc;

    if (item_ct1.get_local_id(2) < ST::cols) {
        ...
        for(int t = 0; t < N_TILES; t++) {
            ...
            for(int i = 0; i < ST::rows; i++) {
                acc += sycl::ext::intel::math::bfloat162float(
                    v[t][sycl::int2{i, col}]);
                o[t][sycl::int2{i, col}] +=
                    sycl::ext::intel::math::float2bfloat16(acc);
            }
        }
        running_sum[col] = acc;
    }
...
inline void cumsum_inplace(ST (&x)[N_TILES], int total_block_idx) {
    constexpr int STRIDE = WORKERS*kittens::WARP_THREADS;

    for(int i = 1; i < N_TILES; i++) {
        #pragma unroll
        for (int j = sycl::ext::oneapi::this_work_item::get_nd_item<3>()
                         .get_local_id(2);
             j < ST::num_elements; j += STRIDE) {
...
template<int WORKERS, kittens::ducks::st::all ST, int N_TILES>
inline void tile_reduce(ST &dst, const ST (&src)[N_TILES]) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    constexpr int STRIDE = WORKERS * kittens::WARP_THREADS;
    ...
    for(int j = 0; j < RESPONSIBLE_ELEMENTS; j++) {
        int idx = item_ct1.get_local_id(2) + j * STRIDE;
        acc[j] = sycl::ext::intel::math::bfloat162float(dst.data[idx]);
    }
    ...
    for(int i = 0; i < N_TILES; i++) {
        ...
        for(int j = 0; j < RESPONSIBLE_ELEMENTS; j++) {
            int idx = item_ct1.get_local_id(2) + j * STRIDE;
            acc[j] += sycl::ext::intel::math::bfloat162float(src[i].data[idx]);
        }
    }
    #pragma unroll
    for(int j = 0; j < RESPONSIBLE_ELEMENTS; j++) {
        int idx = item_ct1.get_local_id(2) + j * STRIDE;
        dst.data[idx] = sycl::ext::intel::math::float2bfloat16(acc[j]);
    }
}

static void mul_slice(rt_bf<16, 16> &reg) {
    ...
    for(int row_offset = 0; row_offset < 2; row_offset++) {
        ...
        /*
        DPCT1108:377: '__shfl_sync' was migrated with the experimental feature
        ...
        */
        bf16 val = dpct::experimental::select_from_sub_group(
            ...
            (target_col % 2 == 0) ? src_val.x() : src_val.y(), src_thread);

        val *= sycl::ext::intel::math::float2bfloat16(0.70710678118);
...
based_linear_attention exceeds 128 bytes and may cause high register pressure.
Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
SYCL_EXTERNAL
void based_linear_attention(const based_globals g,
                            uint8_t *dpct_local) {
    ...
    for (int block = 0; block < n_blocks; block++) {
        ...
        */
        ...
        /*
        DPCT1118:381: SYCL group functions and algorithms must be encountered in
        converged control flow. You may need to adjust the code.
        */
        /*
        ...
        */
        ...
        /*
        DPCT1118:382: SYCL group functions and algorithms must be encountered in
        converged control flow. You may need to adjust the code.
        */
        /*
        DPCT1065:557: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
        ...
        for(int t = 0; t < ACTIVE_TILES; t++) {
            ...
            /*
            DPCT1118:385: SYCL group functions and algorithms must be
            encountered in converged control flow. You may need to adjust the
            code.
            */
            /*
            DPCT1065:560: Consider replacing sycl::nd_item::barrier() with
            sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
            better performance if there is no access to global memory.
            */
            item_ct1.barrier();
            ...
            /*
            DPCT1118:386: SYCL group functions and algorithms must be
            encountered in converged control flow. You may need to adjust the
            code.
            */
            /*
            DPCT1065:561: Consider replacing sycl::nd_item::barrier() with
            sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
            better performance if there is no access to global memory.
            */
            item_ct1.barrier();
        ...
        /*
        DPCT1118:383: SYCL group functions and algorithms must be encountered in
        converged control flow. You may need to adjust the code.
        */
        /*
        DPCT1065:558: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
        ...
        /*
        DPCT1118:384: SYCL group functions and algorithms must be encountered in
        converged control flow. You may need to adjust the code.
        */
        /*
        DPCT1065:559: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();

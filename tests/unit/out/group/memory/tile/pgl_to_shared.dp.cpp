...
static void group_p2s_global_wrapper_2d(const PGL input, const PGL output, const int dev_idx,
                                        uint8_t *dpct_local) {
    ...
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS>::value) {
            ...
            for (int dev_idx = 0; dev_idx < (test::single_run ? 1 : NUM_DEVICES); ++dev_idx) {
                /*
                DPCT1093:54: The "dev_idx" device may be not the one intended
                for use. Adjust the selected device if needed.
                */
                dpct::select_device(dev_idx);
                /*
                DPCT1026:55: The call to cudaFuncSetAttribute was removed
                              ...
                              dpct::get_in_order_queue().submit([&](sycl::handler
                                    ...
                                    cgh.parallel_for(
                                        ...
                                        [=](sycl::nd_item<3> item_ct1) {
                                              group_p2s_global_wrapper_2d<
                                                  test, H, W, NUM_WORKERS,

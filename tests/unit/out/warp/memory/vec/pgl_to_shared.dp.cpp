...
static void p2s_global_wrapper_1d(const PGL input, const PGL output, const int dev_idx,
                                  uint8_t *dpct_local) {
    /*
    DPCT1084:36: The function call "p2s_all_reduce_test::device_func" has
    multiple migration results in different template instantiations that could
    ...
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<S, NUM_WORKERS>::value) {
            ...
            for (int dev_idx = 0; dev_idx < (test::single_run ? 1 : NUM_DEVICES); ++dev_idx) {
                /*
                DPCT1093:73: The "dev_idx" device may be not the one intended
                for use. Adjust the selected device if needed.
                */
                dpct::select_device(dev_idx);
                /*
                DPCT1026:74: The call to cudaFuncSetAttribute was removed

 ...
 * between shared memory and registers and back.
 */
...
inline static void load(RT &dst, const ST &src) {
    ...
    uint32_t shared_addr /*= static_cast<uint32_t>(__cvta_generic_to_shared(&src.data[0]))*/; //NYI
...
/*
DPCT1110:344: The total declared local variable size in device function store
        ...
        for(int j = 0; j < src.width; j++) {
            if constexpr (sizeof(typename ST::dtype) == 2) {
                ...
                if(blit) {
                    #pragma unroll
                    for(int k = 0; k < 4; k++) {
                        reg_tmp[k] = T2{src.tiles[i][j].data[k].y(),

...
struct mamba2_fwd_layout {
    ...
    using a_global = kittens::gl<float, -1, -1, 1, -1, a_vec>;
        /*
        DPCT1128:411: The type "typename mamba2_fwd_template::layout::globals"
        is not device copyable for copy constructor, move constructor, non
        trivially copyable field "Q", non trivially copyable field "K", non
        trivially copyable field "V", non trivially copyable field "O" and non
        trivially copyable field "A" breaking the device copyable requirement.
        It is used in the SYCL kernel, please rewrite the code.
        */
        struct globals {
            q_global Q; k_global K; v_global V; o_global O; a_global A;
        };
        struct input_block    { 
        q_tile q;
        k_tile k;
...
struct mamba2_fwd_template {
	...
	struct consumer {
            ...
            zero(args.state.kv);
                ...
                SYCL_EXTERNAL static bool compute(consumer_compute_args<layout> args) {
            ...
            for(int i = 0; i < 4; i++) {
                ...
                args.state.local_decay.tiles[0][i].data[0].x() =
                    args.scratch.a_cumsum[warpgroupid][base_row + 0] -
                    args.scratch.a_cumsum[warpgroupid][base_col + 0];
                args.state.local_decay.tiles[0][i].data[0].y() =
                    args.scratch.a_cumsum[warpgroupid][base_row + 0] -
                    args.scratch.a_cumsum[warpgroupid][base_col + 1];
                args.state.local_decay.tiles[0][i].data[1].x() =
                    args.scratch.a_cumsum[warpgroupid][base_row + 8] -
                    args.scratch.a_cumsum[warpgroupid][base_col + 0];
                args.state.local_decay.tiles[0][i].data[1].y() =
                    args.scratch.a_cumsum[warpgroupid][base_row + 8] -
                    args.scratch.a_cumsum[warpgroupid][base_col + 1];
                args.state.local_decay.tiles[0][i].data[2].x() =
                    args.scratch.a_cumsum[warpgroupid][base_row + 0] -
                    args.scratch.a_cumsum[warpgroupid][base_col + 8];
                args.state.local_decay.tiles[0][i].data[2].y() =
                    args.scratch.a_cumsum[warpgroupid][base_row + 0] -
                    args.scratch.a_cumsum[warpgroupid][base_col + 9];
                args.state.local_decay.tiles[0][i].data[3].x() =
                    args.scratch.a_cumsum[warpgroupid][base_row + 8] -
                    args.scratch.a_cumsum[warpgroupid][base_col + 8];
                args.state.local_decay.tiles[0][i].data[3].y() =
                    args.scratch.a_cumsum[warpgroupid][base_row + 8] -
                    args.scratch.a_cumsum[warpgroupid][base_col + 9];
            }
            ...
            // // multiply q by decays
            {
                int base_row = warpgroup::warpid()*16 + laneid()/4;
                bf16 top =
                    sycl::ext::intel::math::float2bfloat16(sycl::native::exp(
                        args.scratch.a_cumsum[warpgroupid][base_row + 0]));
                bf16 bottom =
                    sycl::ext::intel::math::float2bfloat16(sycl::native::exp(
                        args.scratch.a_cumsum[warpgroupid][base_row + 8]));
#pragma unroll
            ...
            float total_decay = sycl::native::exp(last_decay);

...
/**
 * @file
 * @brief Warpgroup matrix-multiply accumulate operations. These ops are
 * necessary to achieve full utilization on H100 GPUs.
 */
        ...
        for(int j = 0; j < D::width; j++) {
            ...
            for(int k = 0; k < dst.packed_per_tile; k++) {
                if constexpr(std::is_same_v<typename D::T, float>) {
                    /*
                    DPCT1053:287: Migration of device assembly code is not
                    supported.
                    */
                    //asm volatile(""
                    //             : "+f"(dst.tiles[i][j].data[k].x())::"memory");
                    ///*
                    //DPCT1053:288: Migration of device assembly code is not
                    //supported.
                    //*/
                    //asm volatile(""
                    //             : "+f"(dst.tiles[i][j].data[k].y())::"memory");//NYI
                } else {
                    /*
                    DPCT1053:289: Migration of device assembly code is not
                    supported.
                    */
                    //asm volatile(
                    //    ""
                    //    : "+r"(
                    //        *(uint32_t *)&dst.tiles[i][j].data[k])::"memory");//NYI
                }
            }
        }
    }
 ...
 */
template<typename T=kittens::ducks::default_type> // prevents static assert being instantiated unless called.
static inline void mma_commit_group() {
    KITTENS_CHECK_WARPGROUP
    /*
 ...
 */
template<int N=0>
static inline void mma_async_wait() {
    KITTENS_CHECK_WARPGROUP
    /*
...
template<ducks::rt::row_layout D, ducks::wgmma::input A, ducks::wgmma::input B>
static inline void mm_AtBt(D &d,
                                const A &a,
                                const B &b) {

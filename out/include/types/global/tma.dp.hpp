...
static inline void create_tensor_map(CUtensorMap *tma_map, const typename ST::dtype *src, int batch, int depth, int rows, int cols) {
    ...
    constexpr CUtensorMapDataType tma_format =
        (std::is_same_v<dtype, bf16>         ? CU_TENSOR_MAP_DATA_TYPE_BFLOAT16
         : std::is_same_v<dtype, sycl::half> ? CU_TENSOR_MAP_DATA_TYPE_FLOAT16
    ...
    */
    ...
    /*
    DPCT1009:455: SYCL reports errors using exceptions and does not use error
    codes. Please replace the "get_error_string_dummy(...)" with a real
    error-handling function.
    */
    int res =
        DPCT_CHECK_ERROR(error_string = dpct::get_error_string_dummy(result));
    /*
    DPCT1000:453: Error handling if-stmt was detected but could not be
    rewritten.
    */
    if (result != 0) {
        /*
        DPCT1001:452: The statement could not be removed.
        */
...
static inline CUtensorMap* allocate_and_create_tensor_map(const typename ST::dtype *src, int batch, int depth, int rows, int cols) {
    CUtensorMap *tma_map_d;
    tma_map_d = sycl::malloc_device<CUtensorMap>(1, dpct::get_in_order_queue());
    CUtensorMap tma_map_host; // put it on the stack, why not.
    create_tensor_map<ST>(&tma_map_host, src, batch, depth, rows, cols);
    dpct::get_in_order_queue()
        .memcpy(tma_map_d, &tma_map_host, sizeof(CUtensorMap))
        .wait();
    return tma_map_d;
}
...
static inline void create_tensor_map(CUtensorMap *tma_map, const typename SV::dtype *src, int batch, int depth, int rows, int cols) {
    ...
    */
    ...
    const char *error_string;
    /*
    DPCT1009:459: SYCL reports errors using exceptions and does not use error
    codes. Please replace the "get_error_string_dummy(...)" with a real
    error-handling function.
    */
    int res =
        DPCT_CHECK_ERROR(error_string = dpct::get_error_string_dummy(result));
    /*
    DPCT1000:457: Error handling if-stmt was detected but could not be
    rewritten.
    */
    if (result != 0) {
        /*
        DPCT1001:456: The statement could not be removed.
        */
        std::cerr << "Error in vector TMA descriptor creation: " << error_string
                  << std::endl;
    }
};
...
static inline CUtensorMap* allocate_and_create_tensor_map(const typename SV::dtype *src, int batch, int depth, int rows, int cols) {
    CUtensorMap *tma_map_d;
    tma_map_d = sycl::malloc_device<CUtensorMap>(1, dpct::get_in_order_queue());
    CUtensorMap tma_map_host; // put it on the stack, why not.

    ...
    for(int i = 0; i < total_calls; i++) {
        ...
        if constexpr (assume_aligned) {
            ...
            move<sycl::float4>::ldg(
                tmp, (sycl::float4 *)&src_ptr[row * row_stride + col]);
            move<sycl::float4>::sts(dst.idx(dst_ptr, {row, col}), tmp);
        }
        else {
            if (row + unit_coord.template dim<axis>() < src.template shape<axis>()) {
                ...
                sycl::float4 zeros = {0.f, 0.f, 0.f, 0.f};
                move<sycl::float4>::sts(dst.idx(dst_ptr, {row, col}),
...
#pragma unroll
    for(int i = 0; i < total_calls; i++) {
        ...
        if constexpr (assume_aligned) {
            sycl::float4 tmp;
            move<sycl::float4>::lds(tmp, src.idx(src_ptr, {row, col}));
            move<sycl::float4>::stg(
                (sycl::float4 *)&dst_ptr[row * row_stride + col], tmp);
        }
        else {
...
#pragma unroll
    for(int i = 0; i < total_calls; i++) {
        ...
        if constexpr (assume_aligned) {
            /*
            DPCT1053:49: Migration of device assembly code is not supported.
            */
            asm volatile(
    ...
    /*
    DPCT1026:48: The call to "cp.async.commit_group;

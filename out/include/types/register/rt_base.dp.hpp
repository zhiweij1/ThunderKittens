...
template<typename _T, ducks::rt_layout::all _layout> struct rt_base {
    ...
    static_assert(std::is_same_v<dtype, bf16_2> ||
                      std::is_same_v<dtype, sycl::float2> ||
                      std::is_same_v<dtype, half_2> ||
                      std::is_same_v<dtype, fp8e4m3_4> ||
                      std::is_same_v<dtype, fp8e5m2_4>,
...
using rt_base_hf = rt_base<sycl::half, L>;
#ifdef KITTENS_HOPPER

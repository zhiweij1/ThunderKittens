...
static void p2s_global_wrapper_2d(const PGL input, const PGL output, const int dev_idx,
                                  uint8_t *dpct_local) {
    ...
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS>::value) {
            ...
            for (int dev_idx = 0; dev_idx < (test::single_run ? 1 : NUM_DEVICES); ++dev_idx) {
                /*
                DPCT1093:76: The "dev_idx" device may be not the one intended
                for use. Adjust the selected device if needed.
                */
                dpct::select_device(dev_idx);
                /*
                DPCT1026:77: The call to cudaFuncSetAttribute was removed

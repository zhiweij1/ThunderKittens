...
struct g2s_wrapper_2d {
    ...
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS, axis, args...>::value) {
            ...
            // run kernel
            /*
            DPCT1026:72: The call to cudaFuncSetAttribute was removed because
...
struct group_shared_load_store {
    ...
    template<int H, int W, int NW, gl_t GL, typename axis> static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        o_ref = i_ref; // overwrite the whole thing
    }
    template<int H, int W, int NW, gl_t GL, typename axis> static void device_func(const GL &input, const GL &output,
                                                                                   uint8_t *dpct_local) {

...
static void transpose_global_wrapper_2d(const GTL_I input, GTL_O output) {
                        ...
                        dpct::get_in_order_queue().submit([&](sycl::handler
                              ...
                              cgh.parallel_for(
                                  ...
                                  exp_props, [=](sycl::nd_item<3> item_ct1) {
                                        transpose_global_wrapper_2d<
                                            test, kittens::bf16, H, W,
                                            NUM_WORKERS, GTL_I, GTL_O, args...>(
                                            input, output);
                                  });
                        });
                  }
...
struct test_swap_layout {
    ...
    template<int H, int W, int NW, gl_t GL, kittens::ducks::rt_layout::all L> static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        o_ref = i_ref; // overwrite the whole thing
    }
    template<int H, int W, int NW, gl_t GL, kittens::ducks::rt_layout::all L> static void device_func(const GL input, const GL output) {
...
struct test_transpose {
    ...
    template<int H, int W, int NW, gl_t GTL_I, gl_t GTL_O, kittens::ducks::rt_layout::all L> static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        for(int i = 0; i < H*16; i++)
            for(int j = 0; j < W*16; j++)
                o_ref[i+j*H*16] = i_ref[i*W*16+j];
    }
    template<int H, int W, int NW, gl_t GTL_I, gl_t GTL_O, kittens::ducks::rt_layout::all L> static void device_func(const GTL_I input, GTL_O output) {
        kittens::rt_bf<16*H, 16*W, L> reg_tile;
...
struct test_type_convert {
    template<int H, int W, int NW, typename T2, typename U2> using valid = std::bool_constant<NW == 1 && W*H<=32>; // this is warp-level
        ...
        template<int H, int W, int NW, gl_t GL, typename T2, typename U2> static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        o_ref = i_ref; // overwrite the whole thing
    }
    template<int H, int W, int NW, gl_t GL, typename T2, typename U2> static void device_func(const GL input, const GL output) {
...
struct test_subtile {
    ...
    template<int H, int W, int NW, gl_t GL, typename ST_H> static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        for(int i = 0; i < H*16; i++)
            for(int j = 0; j < W*16; j++)
                o_ref[i*W*16 + j] = i_ref[i*W*16 + j] + float(i/(ST_H::value*16));
    }
    template<int H, int W, int NW, gl_t GL, typename _ST_H> static void device_func(const GL input, const GL output) {
...
struct test_make_causal {
    ...
    pressure.
    */
    template <int H, int W, int NW, gl_t GL, kittens::ducks::rt_layout::all L>

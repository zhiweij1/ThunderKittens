...
template<int _headdim, int _warps> struct rotary_layout {
    ...
    /*
    DPCT1128:338: The type "typename rotary_template<64>::layout::globals" is
    not device copyable for copy constructor, move constructor, non trivially
    copyable field "o", non trivially copyable field "x", non trivially copyable
    field "sin" and non trivially copyable field "cos" breaking the device
    copyable requirement. It is used in the SYCL kernel, please rewrite the
    code.
    */
    struct globals {
        seq_global o, x;
...
template<int _headdim> struct rotary_template {
    ...
    common_setup(common_setup_args<layout> args) {
        if(args.task_iter == 0) {
            /*
            DPCT1064:503: Migrated min call is used in a macro/template
            ...
            */
            args.num_iters =
                sycl::min(
                    ...
                    (int)((
                        int)(args.globals.x.batch() -
                             sycl::ext::oneapi::this_work_item::get_nd_item<3>()
                                     .get_group(1) *
                                 args.globals.batches))) *
                args.globals.x.depth(); // batches*heads handled by block
        }
    ...
    struct producer {
        SYCL_EXTERNAL static void setup(producer_setup_args<layout> args) {
            ...
            /*
            DPCT1064:504: Migrated min call is used in a macro/template
            definition and may not be valid for all macro/template uses. Adjust
            the code.
            */
            args.state.active_warps = sycl::min(
                (int)NUM_CONSUMER_WARPS,
                (int)((int)(args.globals.x.rows() / 16 -
                            sycl::ext::oneapi::this_work_item::get_nd_item<3>()
                                    .get_group(2) *
                                NUM_CONSUMER_WARPS)));
        }
    ...
    struct consumer {
        ...
        /*
        DPCT1110:334: The total declared local variable size in device function

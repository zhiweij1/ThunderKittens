...
void inner_run(fp8e4m3 *d_A, fp8e4m3 *d_B, fp8e4m3 *d_C, size_t M, size_t N,
               size_t K, dpct::dim3 grid, dpct::dim3 block) {
    ...
    globals G{Ag, Bg, Cg};
    /*
    DPCT1049:331: The work-group size passed to the SYCL kernel may exceed the
...
int run_benchmark(size_t M, size_t N, size_t K) {
    ...
    /*
    DPCT1010:495: SYCL uses exceptions to report errors and does not use the
    error codes. The cudaGetLastError function call was replaced with 0. You
    need to rewrite this code.
    */
    cudaStatus = 0;
    /*
    DPCT1000:492: Error handling if-stmt was detected but could not be
    rewritten.
    */
    if (cudaStatus != 0) {
        /*
        DPCT1009:496: SYCL reports errors using exceptions and does not use
        error codes. Please replace the "get_error_string_dummy(...)" with a
        real error-handling function.
        */
        /*
        DPCT1001:491: The statement could not be removed.
        */
    ...
    dpct::get_in_order_queue()
        .memcpy(d_A, h_A_fp8, M * K * sizeof(fp8e4m3))
        .wait();
    dpct::get_in_order_queue()
        .memcpy(d_B, h_B_fp8, K * N * sizeof(fp8e4m3))
        .wait();
    ...
    // Check for CUDA errors
    /*
    DPCT1010:497: SYCL uses exceptions to report errors and does not use the
    error codes. The cudaGetLastError function call was replaced with 0. You
    need to rewrite this code.
    */
    cudaStatus = 0;
    /*
    DPCT1000:494: Error handling if-stmt was detected but could not be
    rewritten.
    */
    if (cudaStatus != 0) {
        /*
        DPCT1009:498: SYCL reports errors using exceptions and does not use
        error codes. Please replace the "get_error_string_dummy(...)" with a
        real error-handling function.
        */
        /*
        DPCT1001:493: The statement could not be removed.
        */
        std::cerr << "CUDA error: " << dpct::get_error_string_dummy(cudaStatus)
                  << std::endl;
        // Optionally, you might want to exit the program or handle the error in some way
    ...
    dpct::get_in_order_queue()
        .memcpy(h_C_fp8, d_C, M * N * sizeof(fp8e4m3))
        .wait();
